...
//		if(bProfiling){ Profiler.EndSample(); }
	...
	public void ConstructRoad_StoreTerrainHistory(bool bDiskOnly = false){
		...
		if(opt_SaveTerrainHistoryOnDisk && TerrainHistory != null && TerrainHistory.Count > 0){
			if(bProfiling){ Profiler.BeginSample("TerrainHistory_Save"); }
	...
	private void UpdateRoad_NoMultiThreading(){
		if(opt_HeightModEnabled || opt_DetailModEnabled || opt_TreeModEnabled){
			if(bProfiling){ Profiler.BeginSample("RoadCon_Terrain"); }
			if(RCS.bTerrainOn || TerrainHistory == null){
		...
		GSDRoad tRoad = this;
		if(bProfiling){ 
			Profiler.BeginSample("RoadCon_RoadPrelim");
		}
		...
		if(bProfiling){ 
			Profiler.EndSample();
			Profiler.BeginSample("RoadCon_Road1");
		}
		EditorProgress = 90;
		GSD.Threaded.RoadCalcs1_static.RunMe(ref RCS);
		if(bProfiling){ Profiler.EndSample(); }
		if(bProfiling){ Profiler.BeginSample("MeshSetup1"); }
		EditorProgress = 92;
		RCS.MeshSetup1();
		if(bProfiling){ 
			Profiler.EndSample();
			Profiler.BeginSample("RoadCon_Road2");
		}
		EditorProgress = 94;
		GSD.Threaded.RoadCalcs2_static.RunMe(ref RCS);
		if(bProfiling){ Profiler.EndSample(); }
		if(bProfiling){ Profiler.BeginSample("MeshSetup2"); }
		EditorProgress = 96;
		RCS.MeshSetup2();
		if(bProfiling){ Profiler.EndSample(); }
